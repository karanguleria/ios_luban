...
extension OrderDetailVC : UITableViewDelegate, UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.orderListDataModel.productsDataModel.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "MyCartTVC") as! MyCartTVC
        let product = self.orderListDataModel.productsDataModel[indexPath.row]
        cell.lbl_productName.text = product.name
        cell.lbl_amount.text = "\(product.actualPrice ?? "0")"
        cell.lbl_productDes.text = product.description
        cell.productImage.sd_setImage(with: URL(string: product.image ?? ""), placeholderImage: UIImage(named: "door"))
        return cell
    }
}
